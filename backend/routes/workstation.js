...
router.post('/', requirePermission('production_create'), (req, res) => {
  try {
    ...
    const existing = req.db.get('SELECT id FROM workstations WHERE code = ?', [code]);
...
router.delete('/:id', validateId, requirePermission('production_delete'), (req, res) => {
  try {
    req.db.run('DELETE FROM workstations WHERE id = ?', [req.params.id]);
    res.json({ success: true });
  } catch (error) {
    console.error('[workstation.js]', error.message);
    res.status(500).json({ success: false, message: '服务器错误' });
...
// 按工位编码查询当前任务列表
router.get('/screen/:code', (req, res) => {
  try {
    const station = req.db.get(`
    ...
    const tasks = processCode ? req.db.all(`
...
router.get('/screen/:code/:poId', (req, res) => {
  try {
    ...
    // 当前工序绑定的物料
    const station = req.db.get('SELECT process_id FROM workstations WHERE code = ?', [req.params.code]);
    let materials = [];
    if (station?.process_id) {
      const pp = req.db.get('SELECT id FROM product_processes WHERE product_id = ? AND process_id = ?', [order.product_id, station.process_id]);
      if (pp) {
        materials = req.db.all(`
    ...
    // 最近巡检记录
    const recentInspections = req.db.all(`
...
// 工位屏幕 — 快捷报工（免鉴权，需填操作人）
router.post('/screen/:code/:poId/report', (req, res) => {
  try {
    ...
    const station = req.db.get(`SELECT w.*, p.id as pid FROM workstations w LEFT JOIN processes p ON w.process_id = p.id WHERE w.code = ?`, [req.params.code]);
    if (!station?.pid) return res.status(400).json({ success: false, message: '工位未绑定工序' });

    const poId = parseInt(req.params.poId);
    const production = req.db.get('SELECT * FROM production_orders WHERE id = ?', [poId]);
    if (!production) return res.status(404).json({ success: false, message: '工单不存在' });

    req.db.transaction(() => {
      // 更新工序记录
      const record = req.db.get('SELECT * FROM production_process_records WHERE production_order_id = ? AND process_id = ?', [poId, station.pid]);
      if (record) {
        req.db.run(`UPDATE production_process_records SET output_quantity = output_quantity + ?, defect_quantity = defect_quantity + ?, operator = ?, status = 'completed', end_time = CURRENT_TIMESTAMP WHERE id = ?`,
          [output_quantity, defect_quantity || 0, operator, record.id]);
      }
      // 更新工单完成数
      req.db.run('UPDATE production_orders SET completed_quantity = completed_quantity + ?, status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
...
// 工位屏幕 — 快捷巡检（免鉴权）
router.post('/screen/:code/:poId/inspect', (req, res) => {
  try {
    ...
    req.db.transaction(() => {
      ...
      if (inspResult === 'fail') {
        req.db.run("UPDATE production_orders SET status = 'quality_hold', updated_at = CURRENT_TIMESTAMP WHERE id = ? AND status = 'processing'", [poId]);
        const processInfo = req.db.get('SELECT name FROM processes WHERE id = ?', [station.pid]);
