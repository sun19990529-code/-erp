...
router.get('/:id', validateId, requirePermission('warehouse_view'), (req, res) => {
  try {
    const order = req.db.get(`SELECT s.*, w.name as warehouse_name FROM stocktake_orders s JOIN warehouses w ON s.warehouse_id = w.id WHERE s.id = ?`, [req.params.id]);
    if (!order) return res.status(404).json({ success: false, message: '盘点单不存在' });
    const items = req.db.all(`SELECT si.*, p.code as product_code, p.name as product_name, p.unit, p.specification FROM stocktake_items si JOIN products p ON si.product_id = p.id WHERE si.stocktake_id = ?`, [req.params.id]);
...
router.post('/', requirePermission('warehouse_create'), (req, res) => {
  try {
    ...
    let stocktakeId;
    req.db.transaction(() => {
      const result = req.db.run(`INSERT INTO stocktake_orders (order_no, warehouse_id, operator, remark) VALUES (?, ?, ?, ?)`,
        [orderNo, warehouse_id, operator || null, remark || null]);
      stocktakeId = result.lastInsertRowid;
      // 自动拉取该仓库所有库存记录
      const inventoryItems = req.db.all(`SELECT product_id, batch_no, SUM(quantity) as total FROM inventory WHERE warehouse_id = ? GROUP BY product_id, batch_no HAVING total > 0`, [warehouse_id]);
      inventoryItems.forEach(inv => {
        req.db.run(`INSERT INTO stocktake_items (stocktake_id, product_id, batch_no, system_quantity) VALUES (?, ?, ?, ?)`,
...
// 录入实际数量
router.put('/:id/items', validateId, requirePermission('warehouse_edit'), (req, res) => {
  try {
    const { items } = req.body;
    const order = req.db.get('SELECT * FROM stocktake_orders WHERE id = ?', [req.params.id]);
    if (!order) return res.status(404).json({ success: false, message: '盘点单不存在' });
      ...
      (items || []).forEach(item => {
        if (item.actual_quantity != null) {
          ...
          if (item.is_new || !item.id) {
            req.db.run('INSERT INTO stocktake_items (stocktake_id, product_id, batch_no, system_quantity, actual_quantity, difference, remark) VALUES (?, ?, ?, ?, ?, ?, ?)',
              [req.params.id, item.product_id, item.batch_no || '-', systemQty, actual, diff, item.remark || null]);
          } else {
            req.db.run('UPDATE stocktake_items SET actual_quantity = ?, difference = ?, remark = ? WHERE id = ? AND stocktake_id = ?',
              [actual, diff, item.remark || null, item.id, req.params.id]);
          }
        }
      });
      req.db.run("UPDATE stocktake_orders SET status = 'counting', updated_at = CURRENT_TIMESTAMP WHERE id = ? AND status = 'draft'", [req.params.id]);
    });
...
// 确认盘点（按差异调整库存）
router.put('/:id/confirm', validateId, requirePermission('warehouse_edit'), (req, res) => {
  try {
    const order = req.db.get('SELECT * FROM stocktake_orders WHERE id = ?', [req.params.id]);
    if (!order) return res.status(404).json({ success: false, message: '盘点单不存在' });
    if (order.status === 'confirmed') return res.status(400).json({ success: false, message: '盘点单已确认' });
    const items = req.db.all('SELECT * FROM stocktake_items WHERE stocktake_id = ?', [req.params.id]);
    // 所有项必须录入实际数量
    ...
    req.db.transaction(() => {
      items.forEach(item => {
        if (item.difference !== 0) {
          const inv = req.db.get('SELECT * FROM inventory WHERE warehouse_id = ? AND product_id = ? AND batch_no = ?',
            [order.warehouse_id, item.product_id, item.batch_no]);
          if (inv) {
            // 直接设置为实际数量
            req.db.run('UPDATE inventory SET quantity = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [item.actual_quantity, inv.id]);
      ...
      req.db.run("UPDATE stocktake_orders SET status = 'confirmed', updated_at = CURRENT_TIMESTAMP WHERE id = ?", [req.params.id]);
    });
...
router.delete('/:id', validateId, requirePermission('warehouse_delete'), (req, res) => {
  try {
    ...
    req.db.transaction(() => {
      req.db.run('DELETE FROM stocktake_items WHERE stocktake_id = ?', [req.params.id]);
      req.db.run('DELETE FROM stocktake_orders WHERE id = ?', [req.params.id]);
    });
    res.json({ success: true });
  } catch (error) {
    console.error('[stocktake.js]', error.message);
