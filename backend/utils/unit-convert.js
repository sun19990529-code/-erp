...
function convertToKg(quantity, unit, productInfo = null) {
  ...
  if (unit === '支') {
    if (productInfo && productInfo.outer_diameter && productInfo.wall_thickness && productInfo.length) {
      ...
      const lenInMeters = new Decimal(productInfo.length).div(1000); // 数据库中长度单位为毫米，计算公式需要米
      const coef = (productInfo.density && new Decimal(productInfo.density).gt(0)) 
        ? new Decimal(productInfo.density) 
        : new Decimal('0.02491');
      const kgPerPiece = od.minus(wt).times(wt).times(coef).times(lenInMeters);
