...
function createTablesIfNotExist(db) {
  ...
  const tables = [
    ...
    { name: 'outsourcing_items', sql: `CREATE TABLE IF NOT EXISTS outsourcing_items (id INTEGER PRIMARY KEY AUTOINCREMENT, outsourcing_order_id INTEGER NOT NULL, product_id INTEGER NOT NULL, quantity INTEGER NOT NULL, returned_quantity INTEGER DEFAULT 0, unit_price REAL DEFAULT 0, pricing_unit TEXT DEFAULT '公斤', pricing_quantity REAL DEFAULT 0, remark TEXT, FOREIGN KEY (outsourcing_order_id) REFERENCES outsourcing_orders(id), FOREIGN KEY (product_id) REFERENCES products(id))` },
  ...
  try {
    ...
    // 双重重量 & 外协计价单位
    checkAndAddCol('inbound_items', 'theoretical_weight', "ALTER TABLE inbound_items ADD COLUMN theoretical_weight REAL;");
    checkAndAddCol('inbound_items', 'actual_weight', "ALTER TABLE inbound_items ADD COLUMN actual_weight REAL;");
    checkAndAddCol('outbound_items', 'theoretical_weight', "ALTER TABLE outbound_items ADD COLUMN theoretical_weight REAL;");
    checkAndAddCol('outbound_items', 'actual_weight', "ALTER TABLE outbound_items ADD COLUMN actual_weight REAL;");
    checkAndAddCol('outsourcing_items', 'pricing_unit', "ALTER TABLE outsourcing_items ADD COLUMN pricing_unit TEXT DEFAULT '公斤';");
    checkAndAddCol('outsourcing_items', 'pricing_quantity', "ALTER TABLE outsourcing_items ADD COLUMN pricing_quantity REAL DEFAULT 0;");
  } catch (e) {
