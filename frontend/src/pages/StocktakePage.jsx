...
const StocktakePage = () => {
  ...
  const confirmStocktake = async (item) => {
    ...
    const res = await api.put(`/stocktake/${item.id}/confirm`, undefined, { invalidate: ['inventory'] });
    if (res.success) {
  ...
  const del = async (item) => {
    ...
    const res = await api.del(`/stocktake/${item.id}`, { invalidate: ['inventory'] });
    if (res.success) load();
          ...
          {data.length === 0 ? (
            ...
            <div key={item.id} className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
              <div className="flex justify-between items-start mb-2">
                <div className="font-bold text-teal-600 font-mono">{item.order_no}</div>
                <span className={`inline-flex px-2 py-1 rounded-md text-xs font-medium ${
                  item.status === 'confirmed' ? 'bg-green-100 text-green-700' : 
                  item.status === 'counting' ? 'bg-blue-100 text-blue-700' : 
                  'bg-gray-100 text-gray-600'
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm text-gray-500 mb-3">
                <div>仓库: <span className="text-gray-700">{item.warehouse_name}</span></div>
                <div>盘点人: <span className="text-gray-700">{item.operator || '-'}</span></div>
                <div className="col-span-2 text-xs text-gray-400">{item.created_at?.slice(0, 10)}</div>
              </div>
              <div className="flex gap-2">
                <button onClick={() => openDetail(item)} className="flex-1 py-2 text-center rounded-lg border border-teal-200 text-teal-600 text-sm font-medium active:bg-teal-50">
                  <i className="fas fa-eye mr-1"></i>详情
                </button>
                {item.status !== 'confirmed' && (
                  <button onClick={() => del(item)} className="px-4 py-2 rounded-lg border border-red-200 text-red-500 text-sm active:bg-red-50">
      ...
      <Modal isOpen={modal.open} onClose={() => setModal({ open: false })} title={modal.mode === 'create' ? '新建盘点单' : `盘点单详情 — ${modal.item?.order_no || ''}`} size="max-w-4xl">
        {modal.mode === 'create' ? (
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm bg-gray-50 rounded-lg p-4">
            ...
            <div className="block md:hidden space-y-2">
              {(modal.item.items || []).map((it, i) => {
                ...
                return (
                  <div key={it.id} className={`rounded-xl p-3 border ${
                    diff > 0 ? 'border-green-200 bg-green-50/30' : diff < 0 ? 'border-red-200 bg-red-50/30' : 'border-gray-100 bg-gray-50/30'
                  }`}>
                    <div className="flex justify-between items-start mb-1">
                      <div>
                        <div className="font-medium text-gray-800 text-sm">{it.product_name}</div>
                        <div className="text-xs text-gray-400 font-mono">{it.product_code} · {it.batch_no}</div>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-2">
                      <div className="text-center bg-white rounded-lg p-2 border border-gray-100">
                        <div className="text-[10px] text-gray-400">系统</div>
                        <div className="font-mono font-bold text-gray-700">{it.system_quantity}</div>
                      </div>
                      <div className="text-center bg-white rounded-lg p-2 border border-gray-100">
                        <div className="text-[10px] text-gray-400">实际</div>
                        {modal.item.status === 'confirmed' ? (
                          <div className="font-mono font-bold text-gray-700">{it.actual_quantity}</div>
                        ) : (
                          <input type="number" step="0.01" value={it.actual_quantity ?? ''} onChange={e => updateItemField(i, 'actual_quantity', e.target.value)}
                            className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-center text-sm font-bold" placeholder="输入" />
                        )}
                      </div>
                    </div>
                    {modal.item.status !== 'confirmed' && (
                      <input type="text" value={it.remark || ''} onChange={e => updateItemField(i, 'remark', e.target.value)}
                        className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm mt-2" placeholder="备注" />
                    )}
                  </div>
                );
              })}
            ...
            {modal.item.status !== 'confirmed' && (
              <div className="p-3 bg-gray-50 border border-gray-200 rounded-lg">
                <div className="text-xs font-medium text-gray-600 mb-2"><i className="fas fa-plus-circle text-teal-600 mr-1"></i>手工补录漏盘物料</div>
                <div className="flex flex-col sm:flex-row gap-2">
                  <select
                    className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white focus:ring-teal-500 focus:border-teal-500"
                  ...
                  <div className="flex gap-2">
                    <input type="text" className="flex-1 sm:w-28 border border-gray-300 rounded-lg px-3 py-2 text-sm" placeholder="批次号(可选)"
                      value={newProduct.batch_no} onChange={e => setNewProduct({ ...newProduct, batch_no: e.target.value })} />
                    <input type="number" step="0.01" className="w-24 border border-gray-300 rounded-lg px-3 py-2 text-sm" placeholder="盘得数量"
                      value={newProduct.actual_quantity} onChange={e => setNewProduct({ ...newProduct, actual_quantity: e.target.value })} />
                    <button onClick={addNewItem} className="px-4 py-2 bg-teal-600 text-white rounded-lg text-sm hover:bg-teal-700 font-medium whitespace-nowrap">
                      添加
                    </button>
                  </div>
                </div>
            ...
            {modal.item.status !== 'confirmed' && (
              <div className="flex flex-col-reverse sm:flex-row justify-end gap-2 pt-2 border-t border-gray-100">
                <button onClick={saveItems} className="w-full sm:w-auto px-4 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium">
                  <i className="fas fa-save mr-2"></i>保存实际数量
                </button>
                <button onClick={() => confirmStocktake(modal.item)} className="w-full sm:w-auto px-4 py-2.5 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors font-medium">
